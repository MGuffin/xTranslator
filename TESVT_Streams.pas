{ {
  ...
  ---------------------
  TwbBaseCachedFileStreamClass and TwbBaseCachedFileStream
  From xEDIT 3.13  : https://github.com/TES5Edit/TES5Edit
  ******************************************************************************* }
...
uses SysUtils, Math, Classes, Windows, TESVT_Ressources;
...
var
  ...
  s: AnsiString;
begin
  if (FCacheEnd - FPosition >= 256) and (FCacheStart >= FPosition) then
  begin
...
procedure TwbBaseCachedFileStream.WriteInt64(aInt64: Int64);
begin
  WriteBuffer(aInt64, SizeOf(aInt64));
end;

procedure TwbBaseCachedFileStream.WriteByte(aByte: Byte);
begin
...
begin
  Result := Windows.CreateFile(PChar(FFileName), GENERIC_READ, FILE_SHARE_READ, nil, OPEN_EXISTING, FlagsAndAttributes, 0);
...
begin
  if (FDisableStreamReadCacheRefCount > 0) or (Count > FCacheSize) then
  begin
...
begin
  Result := Windows.CreateFile(PChar(FFileName), GENERIC_READ or GENERIC_WRITE, 0, nil, CREATE_ALWAYS, FlagsAndAttributes, 0);
  if Result = INVALID_HANDLE_VALUE then
  begin
